...
const Main = ({ language }) => {
  ...
  const clearLabelStyles = () => {
    const dragLabelElements = document.getElementsByClassName(
      "block-drag-label-content"
    );
  ...
  const onDrop = (event) => {
    ...
    switch (type) {
      ...
      case "empty-block-item":
        ...
        newBlockList[blockIndex].children[itemIndex].children = [
          currentItem.data,
        ];
        if (currentItem.type === "move") {
          ...
          if (oldItem.children.length === 1) {
            newBlockList[oldBlockIndex].children[oldItemIndex].children = [
              defaultContentConfig,
            ];
          } else {
            newBlockList[oldBlockIndex].children[oldItemIndex].children =
              oldItem.children.filter(
      ...
      case "drag-over-column":
        {
          ...
          if (currentItem.type === "add") {
            ...
            newCurrentItem = {
              ...currentItem,
              type: "edit",
              index: position === "top" ? Number(index) : index - 1,
            };
          }
      ...
      case "block-item-move":
        {
          ...
          const blockItem =
            newBlockList[blockIndex].children[contentIndex].children;
          let newCurrentItem = deepClone(currentItem);
          if (currentItem.type === "add") {
          ...
          if (currentItem.type === "move") {
            ...
            if (
              ...
              if (oldItemIndex < itemIndex) {
                ...
                blockItem.splice(
                  position === "top" ? Number(itemIndex) : itemIndex - 1,
                  0,
                  moveItem
                );
                newCurrentItem = {
                ...
                const moveItem = oldItem.splice(oldItemIndex, 1)[0];
                blockItem.splice(
                  position === "top" ? Number(itemIndex) : itemIndex,
                  0,
                  moveItem
                );
                newCurrentItem = {
      ...
      switch (event.target.dataset.type) {
        ...
        case "empty-block-item":
          event.target.parentNode &&
            (event.target.parentNode.style.outlineStyle = "");
          break;
        case "drag-over-column":
        default:
          break;
